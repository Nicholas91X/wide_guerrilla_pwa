...
export default function ChallengeScreen({ step }: Props) {
  const { state, chooseOption, continueToNext } = useGame();
      ...
      {hasChosen ? (
        ...
        <div className="flex-1 flex flex-col justify-between">
          ...
          <button
            ...
            className="w-full mt-8 bg-gold text-background font-body font-semibold py-4 rounded-full text-base hover:bg-gold-light active:scale-95 transition-all"
          >
            Continua
          </button>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {stepData.options.map((option, i) => (
              <button
                ...
                className="text-left border border-gold/30 text-foreground font-body text-sm py-3 px-4 rounded-xl hover:border-gold hover:bg-gold/5 active:scale-[0.98] transition-all"
              >
